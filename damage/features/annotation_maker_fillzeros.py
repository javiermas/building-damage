...
class AnnotationMaker_fillzeros(Feature):
    ...
    def _combine_annotations_and_rasters(self, annotation_data, raster_data):
        ...
        annotation_and_raster_data = self.carry_annotations(annotation_and_raster_data)
        annotation_and_raster_data = annotation_and_raster_data\
            .drop(columns=['damage_0', 'damage_3_back', 'damage_3_forward', 'damage_num', 'date'])\
    ...
    def _combine_annotations_and_rasters(self, annotation_data, raster_data):
        ...
        annotation_and_raster_data['damage_0_forw'] = annotation_and_raster_data['destroyed']\
            .apply(lambda x: np.nan if x != 0 else 1)

        annotation_and_raster_data['damage_3'] = annotation_and_raster_data['destroyed']\
            .apply(lambda x: np.nan if x != 1 else 1)

        annotation_and_raster_data['damage_3_back'] = annotation_and_raster_data['destroyed']\
            .apply(lambda x: np.nan if x != 1 else 1)

        annotation_and_raster_data['damage_0_forw'] = annotation_and_raster_data\
            .groupby(['city', 'patch_id'])['damage_0_forw']\
            .ffill()
        annotation_and_raster_data['damage_3_back'] = annotation_and_raster_data\
            .groupby(['city', 'patch_id'])['damage_3_back']\
            .bfill()

        # the two previous fillin will only be used together to fillforward 0s

        annotation_and_raster_data['damage_3'] = annotation_and_raster_data\
            .groupby(['city', 'patch_id'])['damage_3']\
            .ffill()
            # If it's destroyed at time t, it will be destroyed at time t+i, for any i >= 0

        annotation_and_raster_data['damage_0'] = annotation_and_raster_data\
            .groupby(['city', 'patch_id'])['damage_0']\
            .bfill()
             # If it's not at time t, it will not be destroyed at time t-i, for any i >= 0

        # the Nans that have been forward filled with 0s and backward
        # filled with 1s

        #the function below will first fill up the NaN with 0s if there is no 1s pulled backward
        annotation_and_raster_data['destroyed'] = annotation_and_raster_data\
            .apply(self._damage_to_destruction, axis=1)

        annotation_and_raster_data = annotation_and_raster_data\
            .drop(columns=['damage_0', 'damage_0_forw', 'damage_3_back', 'damage_3', 'damage_num', 'date'])\
            .rename(columns={'raster_date': 'date'})\
            .reset_index(drop=True)\
            .sort_values(['city', 'patch_id', 'date'])
        return annotation_and_raster_data.dropna(subset=['image'])
    ...
    def _damage_to_destruction(x):
        if pd.isna(x['damage_3_back']):
            print("entered")
            if x['damage_0_forw'] == 1:
                return 0.
        elif x['damage_3'] == 1:
            return 1.
        elif x['damage_0'] == 1:
